<#
    .Name
        List Group Members and Membership in OU based on Criteria
    .Synopsis

    .Description
        Script file that returns a list of all the other AD users in each group under an Organizational Unit based on criteteria in a text file.
...
$Groups = (Get-AdGroup -filter * -SearchBase "ou=imaging system,dc=unfcsd,dc=unf,dc=edu")
$StudentAccessGroups = (Get-Content C:\temp\StudentAccessGroups.txt) 
...
Foreach ($Group in $Groups) {
    ...
    try{
        ...
        foreach ($Member in $Arrayofmembers) {
            if($Group.Name -in $StudentAccessGroups){
